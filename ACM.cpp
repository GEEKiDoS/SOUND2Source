...
void ProcessAudio(const WCHAR *Input, const WCHAR *Output, int bitRate, int audioType)
{
	HRESULT hr = S_OK;
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = transcoder.OpenFile(Input);

		if (SUCCEEDED(hr))
		{
			//Configure the profile and build a topology.
			hr = transcoder.ConfigureAudioOutput(bitRate, audioType);
		}

		if (SUCCEEDED(hr))
		{
			hr = transcoder.ConfigureContainer(audioType);
		}
		...
		if (SUCCEEDED(hr))
		{
			hr = transcoder.EncodeToFile(Output);
		}
	...
	if (FAILED(hr))
	{
		...
		_snwprintf(formatted, 256, L"Could not create the output file (0x%X).\n", hr);

		MessageBox(NULL, formatted, L"Error Converting.", MB_ICONERROR);
	}
}
...
HRESULT CTranscoder::ConfigureAudioOutput(int bitRate, int audioType)
{
	assert(m_pProfile);
	...
	if (SUCCEEDED(hr))
	{
		for (int i = 0; i < dwMTCount; i++) {

			hr = pAvailableTypes->GetElement(i, &pUnkAudioType);
			if (SUCCEEDED(hr))
			{
			...
			if (SUCCEEDED(hr))
			{
				UINT32 dwSampleRate = 0;
				pAudioType->GetUINT32(MF_MT_AUDIO_SAMPLES_PER_SECOND, &dwSampleRate);

				UINT32 dwBitRate = 0;
				pAudioType->GetUINT32(MF_MT_AUDIO_AVG_BYTES_PER_SECOND, &dwBitRate);

				UINT32 dwChannels = 0;
				pAudioType->GetUINT32(MF_MT_AUDIO_NUM_CHANNELS, &dwChannels);

				printf("Audio Type (%d) SampleRate:%d ByteRate:%d Channels:%d\n", i, dwSampleRate, dwBitRate, dwChannels);

				if (dwSampleRate == 44100 && (dwBitRate == bitRate || audioType == 1) && dwChannels == 2) {
					bFound = true;
					break;
				}
			}
...
;	}
	...
	if (SUCCEEDED(hr))
	{
		hr = pAudioAttrs->SetGUID(MF_MT_SUBTYPE, audioType == 1 ? MFAudioFormat_PCM : MFAudioFormat_MP3);
	}
...
HRESULT CTranscoder::ConfigureContainer(int audioType)
{
	assert(m_pProfile);
	...
	if (SUCCEEDED(hr))
	{
		hr = pContainerAttrs->SetGUID(
			...
			audioType == 1 ? MFTranscodeContainerType_WAVE : MFTranscodeContainerType_MP3
		);
	}
...
HRESULT CTranscoder::Start()
{
	...
	if (FAILED(hr))
	{
		printf("Failed to start the session...\n");
	}
	return hr;
}
